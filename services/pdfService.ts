...
export const generateAttendancePDF = (session: AttendanceSession) => {
  ...
  const presentCount = (session.presentRolls as Set<number>).size;
  ...
  // Roll Number Lists
  const presentList = Array.from(session.presentRolls as Set<number>).sort((a, b) => a - b).join(', ');
  const absentList = Array.from({ length: session.totalStudents }, (_, i) => i + 1)
    .filter(roll => !(session.presentRolls as Set<number>).has(roll))
    .sort((a, b) => a - b)
    .join(', ');
  ...
  doc.text(`Generated by SmartAttend | CODER ROHSIK | ${new Date().toLocaleString()}`, pageWidth / 2, footerY, { align: 'center' });
